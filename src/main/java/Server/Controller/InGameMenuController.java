...
public class InGameMenuController extends Thread {
    ...
    public static void addCardToDeck(Sender sender, GameBoard gameBoard, Soldier card, int playerIndex) {
        gameBoard.getPlayers()[playerIndex].getDeck().add(card);
        sender.sendCommandWithOutResponse("add card to deck " + card.getSendableCardin() + " " + 0);
        if (gameBoard.isGameOnline())
            sender.getUser().getOpponent().getSender().sendCommandWithOutResponse("add card to deck " + card.getSendableCardin() + " " + 1);
        gameBoard.addLog("add card to deck " + card.getSendableCardin() + " 0", playerIndex);
        gameBoard.addLog("add card to deck " + card.getSendableCardin() + " 1", 1 - playerIndex);
    }
